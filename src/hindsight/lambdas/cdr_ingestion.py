...
def handler(event: dict[str, object], context: Any) -> dict[str, object]:
    ...
    max_rows = positive_int("MAX_CDR_ROWS", 10_000)
    with connect_database(database_url()) as connection:
        ingestion = CockroachCdrIngestion(
            connection,
            max_rows=max_rows,
            trust_level=os.getenv("CDR_TRUST_LEVEL", "untrusted"),
        )
    ...
    print(
        json.dumps(
            {
                "event": "cdr_ingestion_complete",
                "request_id": getattr(context, "aws_request_id", None),
                "objects_processed": len(results),
                "replayed_objects": sum(result.replayed for result in results),
                "cdrs_processed": sum(result.cdrs_processed for result in results),
            }
        )
    )
    return response
